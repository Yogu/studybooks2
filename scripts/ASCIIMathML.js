...
ASCIIMathML = new (function(global) {
	...
	function createMmlNode(t, frag) {
		var node = document.createElementNS(
		        "http://www.w3.org/1998/Math/MathML", t);
		if (frag)
	...
	var AMcal = [ 0xEF35, 0x212C, 0xEF36, 0xEF37, 0x2130, 0x2131, 0xEF38,
	        0x210B, 0x2110, 0xEF39, 0xEF3A, 0x2112, 0x2133, 0xEF3B, 0xEF3C,
	        0xEF3D, 0xEF3E, 0x211B, 0xEF3F, 0xEF40, 0xEF41, 0xEF42, 0xEF43,
	        0xEF44, 0xEF45, 0xEF46 ];
	var AMfrk = [ 0xEF5D, 0xEF5E, 0x212D, 0xEF5F, 0xEF60, 0xEF61, 0xEF62,
	        0x210C, 0x2111, 0xEF63, 0xEF64, 0xEF65, 0xEF66, 0xEF67, 0xEF68,
	        0xEF69, 0xEF6A, 0x211C, 0xEF6B, 0xEF6C, 0xEF6D, 0xEF6E, 0xEF6F,
	        0xEF70, 0xEF71, 0x2128 ];
	var AMbbb = [ 0xEF8C, 0xEF8D, 0x2102, 0xEF8E, 0xEF8F, 0xEF90, 0xEF91,
	        0x210D, 0xEF92, 0xEF93, 0xEF94, 0xEF95, 0xEF96, 0x2115, 0xEF97,
	        0x2119, 0x211A, 0x211D, 0xEF98, 0xEF99, 0xEF9A, 0xEF9B, 0xEF9C,
	        0xEF9D, 0xEF9E, 0x2124 ];
	...
	function define(oldstr, newstr) {
		...
		refreshSymbols(); // this may be a problem if many symbols are
							// defined!
	}
	...
	function AMparseSexpr(str) { // parses str and returns [node,tailstr]
		...
		case LEFTBRACKET: // read (expr+)
			...
			if (typeof symbol.invisible == "boolean" && symbol.invisible)
				...
				node = createMmlNode("mo", document
				        .createTextNode(symbol.output));
		...
		case TEXT:
			...
			newFrag.appendChild(createMmlNode(symbol.tag, document
			        .createTextNode(st)));
		...
		case UNARY:
			...
			if (result[0] == null)
				return [
				        createMmlNode(symbol.tag, document
				                .createTextNode(symbol.output)), str ];
			if (typeof symbol.func == "boolean" && symbol.func) { // functions
				...
				if (st == "^" || st == "_" || st == "/" || st == "|"
					...
					node = createMmlNode("mrow", createMmlNode(symbol.tag,
			...
			if (symbol.input == "sqrt") { // sqrt
				...
				node.appendChild(createMmlNode("mo", document
				        .createTextNode(symbol.output)));
				return [ node, result[1] ];
		...
		case BINARY:
			...
			if (result[0] == null)
				return [
				        createMmlNode("mo", document
				                .createTextNode(symbol.input)), str ];
			AMremoveBrackets(result[0]);
	...
	function AMparseIexpr(str) {
		...
		if (symbol.ttype == INFIX && symbol.input != "/") {
			...
			if (result[0] == null) // show box in place of missing argument
				result[0] = createMmlNode("mo", document
				        .createTextNode("\u25A1"));
			else
				AMremoveBrackets(result[0]);
			str = result[1];
			...
			if (symbol.input == "_") {
				...
				if (sym2.input == "^") {
					...
					node = createMmlNode("mrow", node); // so sum does not
														// stretch
				} else {
	...
	function AMparseExpr(str, rightbracket) {
		var symbol, node, result, i, nodeList = [], newFrag = document
		...
		do {
			...
			if (symbol.ttype == INFIX && symbol.input == "/") {
		        ...
		        && (symbol.ttype != LEFTRIGHT || rightbracket) || AMnestingDepth == 0)
			...
			if (len > 0 && newFrag.childNodes[len - 1].nodeName == "mrow"
				...
				if (right == ")" || right == "]") {
					...
					if (left == "(" && right == ")" && symbol.output != "}"
						...
						if (matrix) {
							var row, frag, n, k, table = document
							        .createDocumentFragment();
							for (i = 0; i < m; i = i + 2) {
								...
								for (j = 1; j < n - 1; j++) {
									if (typeof pos[i][k] != "undefined"
									        && j == pos[i][k]) {
										node.removeChild(node.firstChild); // remove
																			// ,
										row.appendChild(createMmlNode("mtd",
										        frag));
										k++;
									} else
	...
	function parseMath(str, latex) {
		var frag, node;
		AMnestingDepth = 0;
		frag = latex ? LMparseExpr(str.replace(/^\s+/g, ""), false, false)[0]
		        : AMparseExpr(str.replace(/^\s+/g, ""), false)[0];
		node = createMmlNode("math", frag);
		if (showasciiformulaonhover) // fixed by djhsu so
			// newline
			node.setAttribute("title", str.replace(/\s+/g, " "));// does not
																	// show in
		// Gecko
		return node;
	}
	...
	function AMautomathrec(str) {
		...
		// of
		// *two* or more tokens, possibly separated by runs of digits and/or
		// space.
		...
		// nnn?
		var letter = "[a-zA-HJ-Z](?=(?:[^a-zA-Z]|$|" + ambigAMtoken
		        + englishAMtoken + simpleAMtoken + "))|";
		var token = letter + texcommand
		        + "\\d+|[-()[\\]{}+=*&^_%\\\@/<>,\\|!:;'~]|\\.(?!(?:\x20|$))|"
		        + ambigAMtoken + englishAMtoken + simpleAMtoken;
		var re = new RegExp("(^|\\s)(((" + token + ")\\s?)((" + token
		        + secondenglishAMtoken + ")\\s?)+)([,.?]?(?=\\s|$))", "g");
		str = str.replace(re, " `$2`$7");
		var arr = str.split(AMdelimiter1);
		var re1 = new RegExp("(^|\\s)([b-zB-HJ-Z+*<>]|" + texcommand
		        + ambigAMtoken + simpleAMtoken + ")(\\s|\\n|$)", "g");
		var re2 = new RegExp("(^|\\s)([a-z]|" + texcommand + ambigAMtoken
		        + simpleAMtoken + ")([,.])", "g"); // removed
		// |\d+
		// for
		// now
	...
	// Greek letters
	        ...
	        // rtag:"mi" causes space to be inserted before a following sin,
			// cos, etc.
	        ...
	        // "|", "\\|", "\\vert" and "\\Vert" modified later: lspace = rspace
			// = 0em
	        {
	            input : "|",
	            tag : "mo",
	        ...
	        // {input:"\\acute", tag:"mover", output:"\u0317", ttype:UNARY,
			// acc:true},
	        // {input:"\\acute", tag:"mover", output:"\u0301", ttype:UNARY,
			// acc:true},
	        // {input:"\\grave", tag:"mover", output:"\u0300", ttype:UNARY,
			// acc:true},
	        // {input:"\\grave", tag:"mover", output:"\u0316", ttype:UNARY,
			// acc:true},
	        {
	            input : "\\grave",
	            tag : "mover",
	        ...
	        // {input:"\\tilde", tag:"mover", output:"\u0303", ttype:UNARY,
			// acc:true},
	        {
	            input : "\\widetilde",
	            tag : "mover",
	...
	function LMgetSymbol(str) {
		...
		for ( var i = 1; i <= str.length && more; i++) {
			...
			if (k < LMnames.length
			        && str.slice(0, LMnames[k].length) == LMnames[k]) {
				match = LMnames[k];
				mk = k;
				i = match.length;
			}
			more = k < LMnames.length
			        && str.slice(0, LMnames[k].length) >= LMnames[k];
		}
		...
		if ("0" <= st && st <= "9")
			...
			tagst = (("A" > st || st > "Z") && ("a" > st || st > "z") ? "mo"
			        : "mi");
		/*
		 * // Commented out by DRW (not fully understood, but probably to do
		 * with // use of "/" as an INFIX version of "\\frac", which we don't
		 * want): //} //if (st=="-" && LMpreviousSymbol==INFIX) { //
		 * LMcurrentSymbol = INFIX; //trick "/" into recognizing "-" on second
		 * parse // return {input:st, tag:tagst, output:st, ttype:UNARY,
		 * func:true}; //}
		 */
		return {
		    input : st,
	...
	function LMparseSexpr(str) { // parses str and returns
		...
		if (symbol.ttype == DEFINITION) {
			str = symbol.output
			        + LMremoveCharsAndBlanks(str, symbol.input.length);
		...
		case UNDEROVER:
			return [
			        createMmlNode(symbol.tag, document
			                .createTextNode(symbol.output)), str, symbol.tag ];
		case CONST:
			var output = symbol.output;
			node = createMmlNode(symbol.tag, document.createTextNode(output));
			return [ node, str, symbol.tag ];
		case LONG: // added by DRW
			node = createMmlNode(symbol.tag, document
			        .createTextNode(symbol.output));
		...
		case STRETCHY: // added by DRW
			...
			node.setAttribute("maxsize", symbol.atval); // don't allow to
														// stretch
			// here
			if (symbol.rtag != null)
			...
			if (symbol.input == "\\begin{array}") {
				...
				if (symbol == null)
					...
					if (symbol.input != "{")
						...
						} while (symbol != null && symbol.input != ""
						        && symbol.input != "}");
				}
		...
		case UNARY:
			...
			if (symbol.input == "\\sqrt") { // sqrt
				...
				if (symbol.input == "\\underbrace"
				        || symbol.input == "\\underline")
					node1.setAttribute("accentunder", "true");
				else
					node1.setAttribute("accent", "true");
				node.appendChild(node1);
				if (symbol.input == "\\overbrace"
				        || symbol.input == "\\underbrace")
		...
		case BINARY:
			...
			if (result[0] == null)
				return [
				        createMmlNode("mo", document
				                .createTextNode(symbol.input)), str, null ];
	...
	function LMparseIexpr(str) {
		...
		if (symbol.ttype == INFIX) {
			...
			if (symbol.input == "_" || symbol.input == "^") {
				...
				tag = null; // no space between x^2 and a following sin, cos,
							// etc.
...
underover = ((sym1.ttype == UNDEROVER) || (node.ttype == UNDEROVER));
				...
				if (symbol.input == "_" && sym2.input == "^") {
					...
					str = res2[1];
					tag = res2[2]; // leave space between x_1^2 and a following
									// sin etc.
					node = createMmlNode(
					        (underover ? "munderover" : "msubsup"), node);
					node.appendChild(result[0]);
	...
	function LMparseExpr(str, rightbracket, matrix) {
		var symbol, node, result, i, tag, newFrag = document
		        .createDocumentFragment();
		do {
			str = LMremoveCharsAndBlanks(str, 0);
		...
		if (symbol.ttype == RIGHTBRACKET) {
			...
			if (symbol != null)
				str = LMremoveCharsAndBlanks(str, symbol.input.length); // ready
																		// to
			// return
			var len = newFrag.childNodes.length;
			if (matrix && len > 0
	...
	function simpleLaTeXformatting(st) {
		st = st.replace(/\$\$((.|\n)*?)\$\$/g,
		        "<p align=\"center\">$\\displaystyle{$1}$</p>");
		st = st
		        .replace(
		...
		st = st.replace(/\\bigskip/g,
		        "<p style=\"margin-bottom:0.5in\">&nbsp;</p>");
		st = st.replace(/\\medskip/g,
		        "<p style=\"margin-bottom:0.3in\">&nbsp;</p>");
		st = st.replace(/\\smallskip/g,
		        "<p style=\"margin-bottom:0.15in\">&nbsp;</p>");
	...
	function ASCIIandgraphformatting(st) {
		...
		// st = st.replace(/<\/?font.*?>/gi,""); // do this only in
		// amath...endamath
		...
		st = st
		        .replace(
		                /([>\n])(Theorem|Lemma|Proposition|Corollary|Definition|Example|Remark|Problem|Exercise|Conjecture|Solution)(:|\W\W?(\w|\s|-|\.)*?\W?:)/g,
		                "$1<b>$2$3</b>");
		st = st.replace(/<embed\s+class\s?=\s?"?ASCIIsvg"?/gi,
		        "<embed class=\"ASCIIsvg\" src=\"" + dsvglocation
		                + "d.svg\" wmode=\"transparent\"");
		st = st
		        .replace(
		                /(?:\\begin{a?graph}|\bagraph|\(:graph\s)((.|\n)*?)(?:\\end{a?graph}|enda?graph|:\))/g,
		                function(s, t) {
			                return "<table><tr><td><div class=\"ASCIIsvg\"><embed class=\"ASCIIsvg\" src=\""
			                        + dsvglocation
			                        + "d.svg\" wmode=\"transparent\" script=\'"
			                        + t
			                                .replace(/<\/?(br|p|pre)\s?\/?>/gi,
			                                        "\n")
			                        + "\'/></div></td></tr></table>"
		                });
	...
	function LMprocessNode(n) {
		...
		if ((st == null || st.indexOf("\$ ") != -1 || st.indexOf("\$<") != -1
		        || st.indexOf("\\begin") != -1 || am || st.slice(-1) == "$" || st
		        .indexOf("\$\n") != -1)
		        && !/edit-content|HTMLArea|wikiedit|wpTextbox1/.test(st)) {
